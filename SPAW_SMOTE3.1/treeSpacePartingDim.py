...
def split_points(data_spreadability, extract_data_num, layer, dim, ax, graph, points, indices, lw=3.0, lo=0.0, hi=5.0 / 6.0,
    ...
    # leaf=true or false
    #print("length: {}".format(len(point_selection)))
    ...
    if leaf:
        ...
        for i in indices:
            ...
            # print(result[num])
            data_spreadability_sum = data_spreadability_sum + data_spreadability[i]

            num = num + 1
        data_spreadability_ave = data_spreadability_sum/(min_count+maj_count)
        #借鉴ADASYN算法
        if maj_count == 0:
            if min_count > 1:
                proportion.append(1/data_spreadability_ave)
            else:
                proportion.append(0)
            # leave_min_flag.append(0)
        elif maj_count > 0:
            if min_count > 1:
                # leave_min_flag.append(1)
                proportion.append(min_count/maj_count/data_spreadability_ave)
            else:
                # leave_min_flag.append(0)
                proportion.append(0)
    ...
    split_points(data_spreadability, extract_data_num, layer, dim, ax, graph, points, indices_a, lw * 0.8, lo, (lo + hi) / 2,
                 visitor, max_splits - 1, draw_splits, (splits, v, a), seed, leaf_size, node_id)
    split_points(data_spreadability, extract_data_num, layer, dim, ax, graph, points, indices_b, lw * 0.8, (lo + hi) / 2, hi,
                 visitor, max_splits - 1, draw_splits, (splits, -v, -a), seed, leaf_size, node_id)
...
def space(leaf_sizes, points, tag, dim, extract_data_num, pic_num, data_spreadability):
    global result
    global tags
    global keys
    global num
    ...
    plots = [  # ('scatter', ScatterVisitor(), 0, False, '', 10),
        ...
        ('tree-full', TreeVisitor(), 999, True, '', leaf_sizes)]

    for tag, visitor, max_splits, draw_splits, seed, leaf_size in plots:
        fn = tag + str(pic_num) + '.png'
        print(fn, '...')
        ...
        # dim = 2  # 维数
        layer = 1  # 生成树的层
        split_points(data_spreadability, extract_data_num, layer, dim, ax, graph, points, range(len(points)), visitor=visitor,
        ...
        graph.draw('pic/'+tag + str(pic_num) + '-graphviz.png')
        # plt.show()
        ...
        #         if leave_min_flag[pro_num] == 1:
        #             proportion_re.append(1)
        #         else:
        #             proportion_re.append(0)
        #     return result, proportion_re
        # else:
        ...
        # proportion_re = []
        # # min_ave_value = len(leave_min_flag)/leave_min_flag.count(1)
        # min_ave_value = 0
        # if sum(proportion)==0:
        #     for pro_num in range(len(proportion)):
        #         if leave_min_flag[pro_num] == 1:
        ...
        #     for pro_num in range(len(proportion)):
        #         if leave_min_flag[pro_num] == 1:
        #             proportion_re.append(proportion[pro_num]+min_ave_value)
        #         else:
        #             proportion_re.append(proportion[pro_num])



        # #print("min_ave_value:{}".format(min_ave_value))
        # for pro_num in range(len(proportion)):
        #     if leave_min_flag[pro_num] == 1:
        #         proportion_re.append(proportion[pro_num]+min_ave_value)
        #     else:
...
if __name__ == '__main__':
    filepath = 'D:\\PycharmObject\\ZQannoy3.3\\dataset\\KEEL\\'
    data, flag = data_read_keel(filepath)
    #print(type(data))
    print(len(flag))
    k = 10
    leaf_sizes = 10
    dim = len(data[0])
    extract_data_num = dataExtract(data, k, 0.6)
    # print(extract_data_num)
    #print(data)
    data = data_str_to_flow(data)
    #print(data)
    space_point = space(leaf_sizes, np.array(data), flag, dim, extract_data_num, 0)

    print(len(space_point))
    #print(len())
