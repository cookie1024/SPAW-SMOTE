...
def split_points(extract_data_num, layer, dim, ax, graph, poly, points, indices, lw=3.0, lo=0.0, hi=5.0/6.0, visitor=None, max_splits=99999, draw_splits=True, splits=None, seed='', leaf_size=1, parent_node_id=None):
    ...
    split_points(extract_data_num, layer, dim, ax, graph, halfplane_a, points, indices_a, lw*0.8, lo, (lo+hi)/2, visitor, max_splits-1, draw_splits, (splits, v, a), seed, leaf_size, node_id)
    split_points(extract_data_num, layer, dim, ax, graph, halfplane_b, points, indices_b, lw*0.8, (lo+hi)/2, hi, visitor, max_splits-1, draw_splits, (splits, -v, -a), seed, leaf_size, node_id)
...
def space(points, tag, extract_data_num):
    global result
    global tags
    global keys
    global num
    ...
    for tag, visitor, max_splits, draw_splits, seed, leaf_size in plots:
        ...
        dim = 2#维数
        layer = 1#生成树的层
        split_points(extract_data_num, layer, dim, ax, graph, plane, points, range(len(points)), visitor=visitor, max_splits=max_splits, draw_splits=draw_splits, seed=seed, leaf_size=leaf_size)
